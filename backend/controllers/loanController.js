...
const createLoanPayment = async (req, res) => {
  ...
  const { second_account, amount } = req.body;
  console.log(loanId, second_account, amount, typeof amount);
  // Validate input parameters
  if (!loanId || !second_account || typeof amount !== "number" || amount <= 0) {
    return res.status(400).json({
      error:
        "Invalid input. Please ensure all fields are correct and the payment amount is positive.",
    });
  }
  ...
  try {
    ...
    const account = await Accounts.findById(second_account).session(session);
    if (!account) {
